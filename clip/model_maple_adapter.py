...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = inputs[0]
        counter = inputs[2]
        layer_depth = inputs[3]
        if compound_prompts_deeper != None:
            if not self.first_layer:
                if layer_depth > 0:
                    ...
                    if not self.text_layer:
                        ...
                        if not (counter > layer_depth - 2):
                            # Remove the outputs produced by learnable tokens of previous layer
                            prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :]
                            # Create/configure learnable tokens of this layer
                            visual_context = compound_prompts_deeper[:, counter:counter + 1, :].squeeze(
                                1)  # extract the correct index
                        ...
                        if not (counter > layer_depth - 2):
                            ...
                            # Create/configure learnable tokens of this layer
                            textual_context = compound_prompts_deeper[:, counter:counter + 1].squeeze()
        ...
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return [x, compound_prompts_deeper, counter, layer_depth]  # return again as a list, so that nn.seq can work
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.VPT_shallow:
            ...
            # visual_ctx = visual_prompt.expand(x.shape[0], -1, -1).half()
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, depth):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0, depth])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
