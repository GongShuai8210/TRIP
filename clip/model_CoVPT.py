...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        #
        # self.VPT_net = nn.Sequential(OrderedDict([
        #     ("linear1", nn.Linear(197,768)),
        #     ("relu", nn.ReLU(inplace=True)),
        #     ("linear2", nn.Linear(768, 8))
        # ]))
        #
    ...
    def forward(self, x: torch.Tensor):
        ...
        #
        x = x.permute(0, 2, 1)
        VPTvis_spe = self.VPT_net(x)
        VPTvis_spe = VPTvis_spe.permute(0, 2, 1)
        x = x.permute(0, 2, 1)
        #
        ...
        if self.VPT_shallow:
            ...
            # x = torch.cat([x, VPTvis_spe], dim=1)
