...
class ResidualAttentionBlock_IVLP(nn.Module):
    ...
    def forward(self, input):
        ...
        x = input[0]
        compound_prompts_deeper = input[1]
        counter = input[2]
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IVLP(width, heads, attn_mask,
                                                                         text_layer, i,
                                                                         design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_IVLP(width, heads, attn_mask,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_prompt, VPT1):
        ...
        output = self.transformer([x, VPT1, 0])
        x = output[0]
