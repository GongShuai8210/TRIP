...
class ResidualAttentionBlock_IVLP(nn.Module):
    def __init__(self, VPT1, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
...
class Transformer(nn.Module):
    def __init__(self, VPT1, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IVLP(VPT1, width, heads, attn_mask, True,
                                                                         text_layer, i,
                                                                         design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_IVLP(VPT1, width, heads, attn_mask, False,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_prompt, VPT1):

        self.transformer = Transformer(VPT1, self.width, self.layers, self.heads,
